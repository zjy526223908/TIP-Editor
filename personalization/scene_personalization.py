...
from ptp_utils import AttentionStore, aggregate_attention, register_unt_attention_control
...
def log_validation(
    ...
    pose_dict = np.load(args.instance_data_dir[0].replace('rgb', 'pose_dict.npy'), allow_pickle=True).item()
    ...
    if args.validation_images is None:
        for i in range(args.num_validation_images):
            ...
            class_labels = torch.cat(
                [
                    pose.view(1, -1),
                    torch.zeros_like(pose.view(1, -1)),
                ],
                dim=0,
            )
            with torch.autocast("cuda"):
                image = pipeline(**pipeline_args, class_labels=class_labels, num_inference_steps=25, generator=generator).images[0]
            images.append(image)
    else:
        for image in args.validation_images:
            ...
            image = pipeline(prompt=args.validation_prompt, class_labels=class_labels,  image=image, strength=0.75, num_inference_steps=50,
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_pos_list = instance_pos_list
    ...
    def __getitem__(self, index):
        ...
        bg_mask_gt = Image.open(str(self.instance_images_path_list[0][index %
             self.num_instance_images_list[0]]).replace('rgb', 'mask'))

        example["bg_mask"] = torch.tensor(np.array(bg_mask_gt) / 255, dtype=torch.float32)

        bg_image = exif_transpose(bg_image).convert("RGB")


        example['ins_pos'] = self.instance_pos_list[0]
        example["instance_images"] = self.image_transforms(bg_image)

        text_inputs = tokenize_prompt(
            self.tokenizer, self.instance_prompt_list[loss_idx], tokenizer_max_length=self.tokenizer_max_length
        )
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    batch = {
        ...
        "loss_idx": [example["loss_idx"] for example in examples],
        "ins_pos": [example["ins_pos"] for example in examples],
        "bg_mask": [example["bg_mask"] for example in examples],
    }
...
def main(args):
    ...
    camera_embedding = ToWeightsDType(
        TimestepEmbedding(16, 1280), weight_dtype
    )
    ...
    train_dataset = DreamBoothDataset(
        ...
        instance_pos_list=args.instance_pos,
        class_data_root_list=args.class_data_dir if args.with_prior_preservation else None,
    ...
    register_unt_attention_control(unet, controller)
    ...
    train_pipeline =  StableDiffusionImg2ImgPipeline.from_pretrained(
        args.pretrained_model_name_or_path,
        tokenizer=tokenizer,
        text_encoder=text_encoder,
        unet=unet,
    ...
    train_pipeline.set_progress_bar_config(disable=True)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                class_labels = torch.cat(
                    [
                        batch["pose"].view(args.train_batch_size, -1),
                        batch["pose"].view(args.train_batch_size, -1),
                    ],
                    dim=0,
                )
                ...
                if batch['loss_idx'][0] == 0:
                    ...
                    last_idx = torch.sum(batch["attention_mask"][0]).item() - 1
                    ...
                    bg_mask_16 = F.interpolate(batch['bg_mask'][0].unsqueeze(0).unsqueeze(0), size=(16, 16),
                                               mode='bilinear', antialias=True).squeeze()

                    ob_pos = batch['ins_pos'][0][0]
                    ...
                    att_loss = args.obpos_loss_weight * att_mask_loss_ob_pos + args.obneg_loss_weight * att_mask_loss_ob_neg
                    ...
                    loss = att_loss + args.prior_loss_weight * prior_loss
            ...
            if batch['loss_idx'][0] == 0:
                ...
                if args.with_prior_preservation:
                    logs.update({"loss_prior": prior_loss.detach().item()})
            else:
                if type(att_mask_loss_ob_pos_16) != int:
                    att_mask_loss_ob_pos_16 = att_mask_loss_ob_pos_16.detach().item()
                logs = {
                    "att_mask_ave_loss_ob_neg_16": att_mask_loss_ob_neg_16.detach().item(),
                    "att_mask_ave_loss_ob_pos_16": att_mask_loss_ob_pos_16,
                    "lr": lr_scheduler.get_last_lr()[0]
                }
                if args.with_prior_preservation:
                    logs.update({"loss_prior": prior_loss.detach().item()})
